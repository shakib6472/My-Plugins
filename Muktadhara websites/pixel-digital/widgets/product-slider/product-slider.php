...
class Elementor_pixel_digital_product_slider extends \Elementor\Widget_Base
{
    ...
    protected function register_controls()
    {
        ...
        $product_categories = get_terms(array(
            'taxonomy' => 'product_cat', // Change 'product_cat' to your specific taxonomy if needed
        ...
        // Loop through categories and create options array
        ...
        $this->add_control(
            ...
            [
                ...
                'label' => esc_html__('Product Status', 'boikotha'),
        ...
        $this->add_control(
			'recent_sale',
			[
				'label' => esc_html__( 'Recent Sale?', 'boikotha' ),
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Yes', 'boikotha' ),
				'label_off' => esc_html__( 'No', 'boikotha' ),
				'return_value' => 'yes',
				'default' => 'no',
			]
		);
    ...
    protected function render()
    {
        ...
        $selected_categoriy = $settings['product_category'];
        $recent_sale = $settings['recent_sale'];
        // Set the query args
        $args = array(
            'post_type'      => 'product',
            'posts_per_page' => -1,
        );
        if($selected_categoriy ) {
            $args['tax_query'][] =array(
                'taxonomy' => 'product_cat', // Change 'product_cat' to your specific taxonomy if needed
                'field'    => 'id', // You can use 'id', 'slug', or 'name' here
                'terms'    => $selected_categoriy,
            );
        }
        // Check if $recent_sale is true, then add meta query for recently sold products
if ('yes' == $recent_sale) {
    $args['meta_query'][] = array(
        'key'     => 'sale_date',  // Replace with your actual custom field key
        'compare' => 'EXISTS',      // Check if the sale date exists (not null)
    );

    // Order by sale date in descending order
    $args['orderby']  = 'meta_value';
    $args['meta_key'] = 'sale_date';  // Replace with your actual custom field key
    $args['order']    = 'DESC';
}
        ...
        if (is_single()) {
            global $product;
            $writes_id = get_field('_book_author_id', get_the_ID());
        }
        if (isset($writes_id)) {
            $args['meta_query'][] = array(
                'key'     => '_book_author_id',
            ...
            <div class="row category-slider">
                ...
                if ($product->have_posts()) {
                    while ($product->have_posts()) {
                        ...
                        $regular_price = $loop_product->get_regular_price();
                        $sale_price = $loop_product->get_sale_price();
                            ...
                            <div class="product_single" style="height: auto;">
                                ...
                                <div class="product_info">
                                    <div class="product_content_same_height_last_sold">
                                        ...
                                        <h4>
                                            <?php if (!empty($sale_price)) { ?>
                                                <span class="price_1"><?php echo wc_price($sale_price); ?></span>

                                                <span class="price_2"><?php echo wc_price($regular_price); ?></span>
                                            <?php } else {
                                            ?>
                                                <span class="price_1"><?php echo wc_price($regular_price); ?></span>
                                            <?php
